...
export type ICategoryOption = Required<DefaultOptionType>

export class CategoryUtil {
  static randomCategoryTreeData: ICategoryOption[] = createMockCategores()

  static getRandomCategoryPair(fromTreeData: ICategoryOption[] = this.randomCategoryTreeData) {
    const treeData = fromTreeData
    ...
    // 返回一级分类和二级分类的对象  
    return {
      firstCategory: randomPrimaryCategory.value,
      secondCategory: randomSecondaryCategory.value
...
function createMockCategores() {
  ...
  // 生成二级分类  
  try {
    const treeData = generateRandomSecondaryCategories(primaryCategories, nouns, adjectives) as unknown[] as ICategoryOption[];
